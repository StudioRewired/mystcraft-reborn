...
@OnlyIn(Dist.CLIENT)
object LinkingBookPreviewCapture {

    private val compressionExecutor = Executors.newSingleThreadExecutor { r ->
        Thread(r, "MystcraftPreviewCompression").also { it.isDaemon = true }
    }

    /**
     * Captures a single frame and delivers the compressed JPEG bytes via [onComplete].
     * [onComplete] is called back on the main thread.
     */
    fun captureFrame(onComplete: (ByteArray) -> Unit) {
        val mc = Minecraft.getInstance()
        val w = PreviewImageComponent.CAPTURE_WIDTH
        val h = PreviewImageComponent.CAPTURE_HEIGHT
        ...
        compressionExecutor.submit {
            try {
                val jpeg = compressToJpeg(pixels, screenW, actualCropH, w, h)
                mc.execute { onComplete(jpeg) }
            } catch (e: Exception) {
                LogUtils.getLogger().warn("Mystcraft: Failed to compress preview frame", e)
            }
        }
    }

    private fun compressToJpeg(
        pixels: ByteArray, srcW: Int, srcH: Int, destW: Int, destH: Int
    ): ByteArray {
        ...
        val param = JPEGImageWriteParam(null).apply {
            ...
            compressionQuality = PreviewImageComponent.JPEG_QUALITY / 100f
        }
